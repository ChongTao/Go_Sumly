...
在 Go 中，变量可以分配在**栈**或**堆**。

- **栈分配**：变量随着函数调用结束自动释放，分配和回收速度非常快。
- **堆分配**：需要使用 GC（垃圾回收），开销相对较大。

**内存逃逸**指的是：**编译器判断某个变量在栈上分配可能不安全，于是将其分配到堆上**，导致产生额外的 GC 压力。
...
1. 返回局部变量指针
   ...
   func foo() *int {
       x := 10
       return &x // x 在函数结束后依然要被使用 -> 堆分配
   }
   ```

2. 变量大小不确定

   如 `slice` 容量过大，编译器无法确定其具体大小，可能逃逸到堆上。

3. 接口类型赋值

   结构体赋值到接口变量时会导致对象逃逸（接口存的实际值在堆中）。
   ...
   func bar() interface{} {
       p := Person{Name: "TranAI"}
       return p // p 会逃逸到堆，因为必须在接口中被引用
   }
   ```

4. 闭包捕获变量

   闭包会把捕获的变量放到堆中，以保证函数调用结束后仍然能访问。

   ```go
   func main() {
       funcs := []func(){}
       for i := 0; i < 3; i++ {
           funcs = append(funcs, func() { fmt.Println(i) }) // i 会逃逸
       }
   }
   ```
...
5. 可变大小的数据结构
   ...
   比如创建很大的切片、map，这些在编译期无法放到栈中，直接在堆上分配。

## 10.2 查看内存逃逸

`go build` 的 `-gcflags` 查看逃逸情况：

```sh
go build -gcflags="-m" main.go  // ./main.go:5:6: moved to heap: x
```

## 10.3 避免内存逃逸

1. 减少返回指针给外部（尽量返回值而不是指针）
...
2. 减少接口的使用
   ...
   如果能用具体类型，就不要用接口，降低逃逸风险。

3. 优化闭包

   避免在闭包中频繁捕获外部变量，可以传参数进去。

4. 控制结构体和切片的大小

   太大的对象容易直接分配到堆，优化设计结构。
